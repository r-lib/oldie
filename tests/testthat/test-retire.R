...
test_that("can't deprecate function multiple times", {
  ...
  foo <- retire(foo, "0.1.0")
  expect_error(retire(foo, "0.1.0"), "Function `foo` is already deprecated")
})

test_that("replacement function must be supplied as symbol", {
  foo <- function() "returned"
  expect_error(retire(foo, "0.1.0", 2), "must be a symbol")
})
...
test_that("replaced arguments must be symbols", {
  ...
  expect_error(retire(foo, "0.1.0", arg = foo()), "must be symbols")
})

test_that("replaced arguments must be named", {
  foo <- function(foo) "returned"
  expect_error(retire(foo, "0.1.0", foo = bar, replaced), "must be named")
})

test_that("replaced arguments must exist in function", {
  foo <- function(foo, bar) "returned"
  expect_error(retire(foo, "0.1.0", bar = other), "Can't find replacement")
})

test_that("can't deprecate the same argument twice", {
  foo <- function(foo) "returned"
  foo <- retire(foo, "0.1.0", bar = foo)
  expect_error(retire(foo, "0.1.0", bar = foo), "`bar` has already been deprecated")

  foo <- retire(foo, "0.1.0", baz = foo)
  expect_error(retire(foo, "0.1.0", bar = foo, baz = foo), "`bar` and `baz` have already been deprecated")
})

test_that("new arguments are reassigned if old is supplied", {
  foo <- function(new) list(new, old)
  foo <- set_env(foo, ns_env("rlang"))
  foo <- retire(foo, "99.9.0", old = new)
...
test_that("deprecated arguments are stored in attributes", {
  ...
  foo <- retire(foo, "0.1.0", old1 = new1, old2 = )
  foo <- retire(foo, "0.2.0", old3 = new3)

  depr_args <- retired_args(foo)
...
test_that("new arguments are added without defaults", {
  ...
  fmls <- formals(retire(foo, "0.1.0", old = ))
  expect_identical(as.list(fmls), alist(new = , old = ))

  replaced_fmls <- formals(retire(foo, "0.1.0", old = new))
...
test_that("can add mark existing argument as deprecated", {
  ...
  expect_error(regexp = NA, retire(foo, "0.1.0", old = ))
})
